  ...
  return (
    <div className="h-full overflow-y-auto bg-[#f9faf6] font-sans pb-12 print:bg-white print:pb-0">
      ...
      <div className="bg-[#f9faf6] p-4 sticky top-0 z-50 print:hidden flex items-center justify-between max-w-4xl mx-auto">
        <button onClick={handleBack} className="flex items-center gap-2 text-[#0B2559] hover:text-[#D4AF37] font-semibold text-sm transition-colors bg-white border border-[#E8D399] px-4 py-2 rounded-full shadow-sm cursor-pointer">
          <ArrowLeft size={16} /> Back
        </button>
        <div className="flex items-center gap-2">
          <button
            onClick={downloadPdf}
            className="flex items-center gap-2 bg-[#0B2559] text-[#D4AF37] border border-[#D4AF37] px-5 py-2 rounded-full font-bold text-sm shadow-md hover:bg-[#164AAB] transition-colors cursor-pointer"
          >
