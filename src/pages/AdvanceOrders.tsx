...
const inputClass = 'w-full rounded-xl border border-[#E5E7EB] bg-white px-3.5 py-2.5 text-sm text-[#0F172A] outline-none transition focus:border-[#7e22ce] focus:ring-2 focus:ring-violet-100'
...
export default function AdvanceOrders({ onOrderCompleted }: AdvanceOrdersProps = {}) {
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[.18em] text-violet-600">Separate from sales</p><h2 className="text-2xl font-black text-[#0F172A]">Advance Orders</h2><p className="mt-1 text-sm text-[#6B7280]">Deposits never count as revenue. Full order value is recognized only after final payment.</p></div><div className="flex gap-2"><button onClick={() => void load()} className="rounded-xl border bg-white p-3 text-[#647064]" title="Refresh"><RefreshCw size={18}/></button></div></div>
              ...
              filtered.map(order => (
                <tr key={order.id} className="hover:bg-violet-50/30 transition-colors">
                  ...
                  <td className="max-w-[180px] px-4 py-3.5 align-middle">
                    <p className="truncate font-semibold text-[#0F172A]">{order.product_name}</p>
